...
#     Returns:
#       total_loss : [batch]
#       loss_per_residue : [batch, length]
#     """
#     B, L, C = log_probs.shape
#     mask_f = mask.float()

#     # ---- standard CCE ----
#     S_one_hot = torch.nn.functional.one_hot(S, C).float()
#     loss_per_residue = -(S_one_hot * log_probs).sum(-1)  # [B, L]

#     cce = torch.sum(loss_per_residue * mask_f, dim=-1) / (
#         torch.sum(mask_f, dim=-1) + 1e-8
#     )  # [B]
...
#     # uniform prior
#     log_uniform = -torch.log(torch.tensor(C, device=log_probs.device, dtype=log_probs.dtype))

#     # KL(p || U) = sum p (log p - log U)
#     kl_per_residue = (probs * (log_probs - log_uniform)).sum(-1)  # [B, L]

#     kl = torch.sum(kl_per_residue * mask_f, dim=-1) / (
#         torch.sum(mask_f, dim=-1) + 1e-8
#     )  # [B]

#     total_loss = cce + lam_kl * kl
#     return total_loss, loss_per_residue
def write_full_PDB(
    save_path: str,
    X: np.ndarray,
    X_m: np.ndarray,
